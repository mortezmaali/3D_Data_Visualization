...
figure('Name', 'Color Gamuts in CIELAB Space', 'NumberTitle', 'off', 'WindowState', 'maximized');
hold on;
...
% Plot convex hulls to outline the boundaries of each color space
...
% Rotate the plot indefinitely until the figure is closed
while ishandle(gcf)
    for az = 0:360
        view(az, 30); % Rotate the view angle
        pause(0.05); % Pause to control rotation speed
        if ~ishandle(gcf) % Check if figure is closed
            break;
        end
    end
end
